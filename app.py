...
# Create a tokenizer for the correct model
...
(2) The text of a valid Overpass API query that can be used to answer the question. The query should be enclosed by three backticks on new lines, denoting that it is a code block. and should NOT contain comments with backslashes.
...
def query_overpass(query):
    ...
    try:
        print(f"{datetime.datetime.now()}overpass response: {response.json()}")
        return response.json()
    except Exception as e:
        print(f"exception querying overpass: {e}")
    ...
    with col1:
        ...
        if st.button("Ask"):
            response = openai.ChatCompletion.create(
                model=MODEL,
                messages=[{"role": "user", "content": CHAT_TEMPLATE.format(history=st.session_state.chat_history, human_input=chat)}]
            )
            ...
            if "```" in response["choices"][0]["message"]["content"]:
                st.session_state.overpass_query = response["choices"][0]["message"]["content"].split("```")[1]
            else:
                st.session_state.overpass_query = None
            ...
            with col2:
                ...
                if st.session_state.overpass_query:
                    ...
                    if response and "elements" in response and len(response["elements"]) > 0:
                        ...
                        while lat == 0 and lon == 0:
                            try:
                                lat = response["elements"][element]["lat"]
                                lon = response["elements"][element]["lon"]
                            except:
                                pass
                            element += 1
                        ...
                        if query_reader_prompt_tokens < 1500:
                            ...
                            response = openai.ChatCompletion.create(
                                model=MODEL,
                                messages=[
                                    {"role": "user", "content": query_reader_prompt}
                                ],
                                temperature=0.5,
                                max_tokens=2048,
                                top_p=1,
                        ...
                        st.write(f"overpass was unable to find any usefull locations: {response}")
